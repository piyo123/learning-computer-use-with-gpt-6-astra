...
import json
import pyautogui
import pyperclip # PyAutoGUI が日本語入力が苦手なため、コピー&ペーストさせる
...
def run_computer_use(prompt, model=MODEL_DEPLOYMENT_NAME):
    ...
    # =========================================================
    # PyAutoGUI が日本語入力が苦手なため、あるいはキーボード配列が日本語ではないため
    # =========================================================
    def paste_text(text):
        pyperclip.copy(text)
    ...
    tools = [
        {
            ...
            "description": (
                ...
                "log(value), display(PIL_image) and and paste_text(text). "
                "Use paste_text(text) when entering URLs or other text that must be entered exactly."

                "Inspect the screen with display(pyautogui.screenshot()) before acting. Use screenshot "
                ...
                "Use Chrome in Guest mode whenever you need to access the web."
                "When navigating to a specified URL, always enter the URL directly into Chrome's address bar."
                "Never enter a URL into the Windows Run dialog."

                "When you open some application windows, move it to on the right half of the screen."

                "Once you have done the task, close the application you opened."
                "Do not close any other applications."
            ),
